...
class train_model(AImodelForex):
    ...
    def train_model_Tp_Sl(self):
        ...
        for  typeOfRun in  ['bear', 'bull']:
            ...
            for targetVal in [f"{typeOfRun}_sl",f"{typeOfRun}_tp"]:
                self.__SaveLog(message=f"start building {targetVal}", typeOFlog=2)
                ...
                path = os.path.join(self.mainPath,
                                    "model",
                                    f'{targetVal}_{self.intervalTime}_{self.nextTime}_{self.timeframe}_{self.currentTime()}.h5')

                self.__saved_file_check(message=f"{targetVal}", path=path, typeOFlog=2)
    ...
    def train_model(self):
        ...
        self.__SaveLog(message="building Binary model",typeOFlog=2)
        ...
        history = model.fit(
            ...
            validation_data=([self.X_val_preProcessed_binaryModel,
        ...
        model.save(rf"{path}")
        self.__saved_file_check(message=f"Binary_model", path=path, typeOFlog=2)
